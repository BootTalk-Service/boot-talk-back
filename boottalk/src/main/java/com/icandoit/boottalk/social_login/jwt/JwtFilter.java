...
import static com.icandoit.boottalk.libs.exception.ErrorCode.INVALID_TOKEN;
...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (requestURI.startsWith("/swagger-ui/")
            ...
            || requestURI.startsWith("/connection")
        ) {
        ...
        log.info("token info : {}, {}, {}", userDetails.getName(),
            userDetails.getAuthorities().stream().map(
                GrantedAuthority::getAuthority).collect(Collectors.toList()),
            userDetails.getServiceUserId());
